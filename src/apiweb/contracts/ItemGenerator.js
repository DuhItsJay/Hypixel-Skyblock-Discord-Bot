...
const parseItems = async function (base64, db) {
	...
	for (const item of items) {
		if (item.tag?.ExtraAttributes?.id != undefined) {
			let itemName = datacrunch.getRawLore(item.tag.display.Name)
			...
			if (itemId == 'enchanted_book' && ExtraAttributes.enchantments) {
				...
				if (enchants.length == 1) {
					...
					itemName = datacrunch.capitalize(`${enchants[0]} ${value}`)
				}
			}
			...
			if (ExtraAttributes.gems) {
				const gems = datacrunch.parseItemGems(ExtraAttributes.gems)
